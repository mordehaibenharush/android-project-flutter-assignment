...
class _RandomWordsState extends State<RandomWords> {
  ...
  Widget build(BuildContext context) {
    ...
    if (Provider.of<AuthRepository>(context, listen: true).status == Status.Authenticated) {
      firestore = FirestoreRepository(userId: Provider.of<AuthRepository>(context, listen: false).user?.uid);
    ...
    return Consumer<AuthRepository>(builder: (context, auth, child)
    {
      return Scaffold( // Add from here...
        ...
        body: _buildStreamedSuggestions(),
      );
    });
  }
                        ...
                        return AlertDialog(
                          ...
                          actions: <Widget>[
                            TextButton(
                                onPressed: () {
                                  firestore?.removeSavedWordPair(pair);
                                  if(Provider.of<AuthRepository>(context, listen: false).status == Status.Unauthenticated)
                                    setState(() {_saved.remove(pair);});
                                  else _saved.remove(pair);
